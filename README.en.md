...
> Enable C18 validation with `ENFORCE_MISSION_TOKEN_VALIDATION=true`. Default (false): an **invalid** token is logged and not opposed — zero impact in standalone mode without mcp-mission.
> ⚠️ *Corrected in v0.16.3 (#154): "continue" was misleading.* Without enforcement, a validation failure leaves `mission_id` empty, so the composite key matches nothing and consumption is **refused** at the registry. More importantly, anything expressing **our own inability** — key service unreachable, audience unresolved, validator not initialised — is refused in **both postures**, as `backend_unavailable` or `misconfigured`: `enforce` decides whether an invalid token is fatal, it does not decide whether we tell the truth about why we refused.
...
**License**: Apache 2.0 | **Author**: Cloud Temple | **Version**: 0.16.3
