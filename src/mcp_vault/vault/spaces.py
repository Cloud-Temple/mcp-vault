...
async def create_space(space_id: str, description: str = "") -> dict:
    """
    Crée un espace vault (mount KV v2 dans OpenBao).

    Args:
        space_id: Identifiant unique (utilisé comme mount path)
    ...
    try:
        client.sys.enable_secrets_engine(
            backend_type="kv",
            path=space_id,
            description=description or f"Vault space: {space_id}",
            options={"version": "2"},
        )
        logger.info(f"✅ Space créé: {space_id}")
        return {"status": "created", "vault_id": space_id, "description": description}
    except Exception as e:
        if "existing mount" in str(e).lower() or "path is already in use" in str(e).lower():
            return {"status": "error", "message": f"L'espace '{space_id}' existe déjà"}
        logger.error(f"❌ Erreur création space {space_id}: {e}")
...
async def get_space_info(space_id: str) -> dict:
    ...
    try:
        mounts = client.sys.list_mounted_secrets_engines()
        mount_key = f"{space_id}/"
        mount_info = mounts.get("data", mounts).get(mount_key)

        if not mount_info:
            return {"status": "error", "message": f"Espace '{space_id}' non trouvé"}

        # Compter les secrets
        secret_count = 0
        try:
            secrets = client.secrets.kv.v2.list_secrets(path="", mount_point=space_id)
        ...
        logger.error(f"❌ Erreur info space {space_id}: {e}")
        return {"status": "error", "message": str(e)}
...
async def delete_space(space_id: str) -> dict:
    ...
    try:
        client.sys.disable_secrets_engine(path=space_id)
        logger.info(f"🗑️ Space supprimé: {space_id}")
        return {"status": "deleted", "vault_id": space_id}
    except Exception as e:
        logger.error(f"❌ Erreur suppression space {space_id}: {e}")
        return {"status": "error", "message": str(e)}
