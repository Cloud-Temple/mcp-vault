    ...
    if (tokens.length === 0) {
        ...
        for (const t of tokens) {
            html += `<tr>
                ...
                <td>${t.allowed_resources && t.allowed_resources.length ? t.allowed_resources.map(r => `<code style="font-size:0.75rem">${esc(r)}</code>`).join(', ') : '<span style="color:var(--muted)">tous</span>'}</td>
                ...
                <td>${!t.revoked ? `<button onclick="revokeToken('${esc(t.hash_prefix)}')" class="btn btn-danger btn-sm">Révoquer</button>` : ''}</td>
            </tr>`;
        }
