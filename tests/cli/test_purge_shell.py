...
# -*- coding: utf-8 -*-
"""
Tests CLI shell — token purge-revoked (issue #50).

Opération destructrice : on verrouille le comportement FAIL-CLOSE du shell
(découvert manquant en revue) :
...
(json envoyé), ce qui prouve l'absence de POST destructif dans les cas refusés.
"""
...
class _FakeHttp:
    ...
    def __init__(self, queue):
        ...
        self.posts = []  # liste des payloads json POSTés
...
def test_shell_purge_older_than_non_entier_ne_poste_rien():
    fake = _run("purge-revoked --older-than abc --yes", queue=[])
    assert fake.posts == []  # fail-close : aucune requête (ni dry-run ni purge)


def test_shell_purge_older_than_manquant_ne_poste_rien():
    fake = _run("purge-revoked --older-than --yes", queue=[])  # "--yes" pris comme valeur invalide
    assert fake.posts == []


def test_shell_purge_older_than_eof_ne_poste_rien():
    fake = _run("purge-revoked --older-than", queue=[])  # --older-than en fin de ligne (EOF)
    assert fake.posts == []


def test_shell_purge_dry_run_en_echec_pas_de_purge_effective():
    fake = _run("purge-revoked --yes", queue=[{"status": "error", "message": "boom"}])
...
def test_shell_purge_yes_execute_apres_dry_run_ok():
    fake = _run("purge-revoked --yes", queue=[
...
def test_shell_purge_older_than_valide_propage_la_retention():
    fake = _run("purge-revoked --older-than 7 --yes", queue=[
