...
- **Fuite de droits corrigée (defense-in-depth)** — la fiche d'un vault (`GET /admin/api/vaults/{id}`) ne divulgue plus **ni les noms ni la cardinalité** des secrets (`secret_keys` retiré ; `get_space_info(count=False)` → aucun `list` indirect) : un token autorisé sur `vault_info` mais **interdit** sur `secret_list` n'apprend rien du contenu. Le listing (racine incluse) passe désormais **toujours** par le canal contrôlé (`check_policy("secret_list")` + `check_path_policy`), et la console affiche le nombre d'entrées à partir de ce listing.
- **Contournement de policy fermé (« validate vs use »)** — le préfixe de listing est **canonicalisé avant** `check_path_policy` (`normalize_list_path`, côté console **et** agent MCP), pour que le contrôle de droits et l'appel OpenBao portent sur la **même** valeur. Sinon `?prefix=%2F` (→ `/`), autorisé par une policy sur `/`, listait la racine (`""`). Le préfixe `/` seul est rejeté ; une erreur de listing → cardinalité **omise**, jamais un « 0 » trompeur.
- **Compteur honnête** — l'ancien « Secrets : N » comptait en réalité les **entrées de premier niveau** (dossiers compris). Libellé corrigé en « Entrées ». La **fiche** affiche le nombre issu du listing contrôlé ; le **tableau des vaults** et le **tableau de bord** utilisent le nouveau champ `root_entries_count` (sémantique explicite, sans parcours récursif). `secrets_count` est conservé (compat MCP `vault_info` / CLI) avec sa sémantique documentée (entrées, pas feuilles).
...
- **Tests** — `tests/test_secrets_folder_nav_81.py` (22 tests unitaires : validation canonique, canonicalisation `normalize_list_path` + non-contournement `?prefix=/`, routage, fuite noms **et** cardinalité, comptage `count=False`/erreur≠0, `domId` bijectif sous Node) + `tests/test_secrets_folder_nav_openbao_81.py` (5 tests d'intégration OpenBao réel : dossiers/feuilles, clés relatives, collision feuille+dossier, lecture d'un dossier, préfixes piégés rejetés). Non-complaisance vérifiée par sabotage (revue Codex).
